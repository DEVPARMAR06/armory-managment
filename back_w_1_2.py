...
class w_1_2:
    def __init__(self,root):
        self.root=root
        self.root.title("ADD-ALLOTTED-GUN")
...
#we use lable as a back ground image
    ...
    def enter(self):
        if self.check_Null():
            ...
            if ch[0]:
                if ch[3]:
                    tkmb.showinfo(title='Message Box', message='RETURN QUANTITY IS NOT GRATER THEN ALLOTED!!!', icon='info')
                else:
                    if ch[4]:
                        f=open('gun_allotment.txt','r')
                        list_of_lines=f.readlines()
                        print(list_of_lines)
                        list_of_lines.pop(ch[2])
                        f=open('gun_allotment.txt','w')
                ...
                tkmb.showinfo(title='Message Box', message='ALLOTMENT NOT FOUND!!!', icon='info')
    ...
    def check(self):
        ...
        for line in f.readlines():
            d=line.split("|")
            if e ==d[0].strip(" ") and a == d[1].strip(" ") and b == d[2].strip(" "):
                f.close()
                if c==int(d[3].strip(" ")):
                    return True,str(int(d[3].strip(" "))-c),line_no,False,True
                if c>int(d[3].strip(" ")):
                    return True,str(int(d[3].strip(" "))-c),line_no,True,False
                else: 
                   return True,str(int(d[3].strip(" "))-c),line_no,False,False
            line_no=line_no+1
        else:
            return False,0 
    def check_Null(self):
    ...
    def update_data_base(self):
       ...
       # d=self.id.get()
        ...
        for line in f1.readlines():
            d1=line.split("|")
            if a == d1[0].strip(" ") and b == d1[1].strip(" "):
                f1.close()
                f1=open('gun_database.txt','r')
                list_of_lines=f1.readlines()
                print(list_of_lines)
                list_of_lines[ln]=a+" | "+b+" | "+str(int(d1[2].strip(" "))+c)+"\n"
                f1=open('gun_database.txt','w')
